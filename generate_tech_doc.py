...
def build_doc():
    ...
    heading(doc, '2.2 交互流程（三阶段分支逻辑）', 2)
    p(doc, '交互流程按三个阶段推进，输入框灰字随阶段切换：')
    for k, v in [
        ('阶段0 · 待输入商品', '灰字「请输入产品名称」。被试输入本品类普通商品后，Agent 输出核心操纵文案'
                       '（协作组以「加入我们，让我们一起为环保做出贡献！」开头；非协作组以「请选择绿色消费！」开头），'
                       '灰字切换为「询问本店可购买的商品」。'),
        ('阶段1 · 待询问商品信息', '灰字「询问本店可购买的商品」。被试输入「本店有什么商品」等询问后，'
                        'Agent 输出 A（非环保产品）/ B（环保产品）的外观与价格信息，灰字切换为'
                        '「您现在可以询问两种商品的具体信息」。'),
        ('阶段2 · 自由问答', '灰字「您现在可以询问两种商品的具体信息」。此后进入自由问答：'
                      '① 追问检测（优点/缺点/用途/价格/材质/环保意义等）→ 从 A/B 知识库回答并自然收尾；'
                      '② 环保款检测 → 提示改输普通版本；③ 询问商品 → 重新展示外观价格；'
                      '④ 普通商品 → 重新展示干预文案；⑤ 兜底报错（按品类）。'),
    ]:
        para = doc.add_paragraph()
        r1 = para.add_run(k)
        set_font(r1, bold=True)
    ...
    heading(doc, '4.2 阶段0回复（输入本品类普通款 → 核心操纵文案）', 2)
    p(doc, '阶段0输入普通商品后，Agent 输出核心操纵文案（干预文案）。12 条文案逐字对照《实验参考文本》：')
    ...
    heading(doc, '4.3 阶段1回复（询问商品 → A/B 外观与价格）', 2)
    p(doc, '阶段1询问商品后，Agent 输出「本店有两种X售卖：」+ A（非环保产品）名称/外观/价格 + B（环保产品）名称/外观/价格。')
    for cat in GA.CATEGORIES:
        p(doc, '【%s】%s' % (cat['category'], GA.build_product_info(cat)), size=9.5)

    heading(doc, '4.4 追问回答（阶段2，示例：优点 / 价格 / 对比 / 推荐）', 2)
